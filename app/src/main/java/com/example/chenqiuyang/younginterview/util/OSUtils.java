...
public class OSUtils {
    ...
    private static ROM initRomType() {
        ...
        try {
            ...
            for (String key : buildProperties.stringPropertyNames()) {
                ...
                buildProperties.put(newKey, newValue);
            }
    ...
    /**
     * 上报给服务器用于判断机型处理的
     * RomName#RomVersion#AndroidVersion
     * @return
     */
    public static String getAuthUploadRomStr() {
        ROM rom = getRomType();
        String romVersion = Build.VERSION.RELEASE;
        if (!TextUtils.isEmpty(rom.getVersion())) {
            romVersion = rom.getVersion();
        }
        return String.format("%s#%s#%d", rom.toString(), romVersion, Build.VERSION.SDK_INT);
    }
}
